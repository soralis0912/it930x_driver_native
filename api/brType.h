...
typedef enum {
	...
	UART_BAUDRATE_57600 = 245,
	UART_BAUDRATE_115200 = 250,
} UartBaudRate;
 ...
 */
typedef enum {
    PID_FILTER_MODE_DISABLE = 0,
    PID_FILTER_MODE_PASS = 1,
	PID_FILTER_MODE_BLOCK = 2,
} PID_FILTER_MODE;

typedef struct {
	PID_FILTER_MODE pidMode;
	Word			pid[64];
	Byte			enable[64];
} SimplePidTable;
...
typedef struct {
    ...
    BrValueSet*         timing;    
    ///IT9300's    
    Byte			receiver_chip_number[4];    
    Bool 			booted;
    Dword 			testnember;
    Byte 			bootflag;
    Word			it9300user_usb20_frame_size_dw;
} Endeavour;
