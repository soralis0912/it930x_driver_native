...
unsigned long omega_init(IN  Demodulator*	demodulator)
{
    ...
    int temp = 0;
	//demod = (DefaultDemodulator*) Afatech_OMEGA_demodulator;	
	demod = (DefaultDemodulator*) demodulator;
    for(temp= 0;temp<9;temp++)
    {
        demod->gc_fN_min[temp] = c_fN_min[temp];
    }


	/*----- read -----*/
	error = omega_rmr(demodulator,0xEC86, 1, buf);		/*p_reg_p_tsm_init_clock_mode*/
	if (error) goto exit;
	error = omega_rmr(demodulator,0xED03, 1, &buf[1]);	/*p_reg_p_fbc_n_code*/
	if (error) goto exit;
	error = omega_rmr(demodulator,0xED23, 2, &buf[2]);	/*r_reg_r_fbc_m_bdry_7_0*/
	if (error) goto exit;
	...
	while (m_bdry == 0 && cnt < OMEGA_TUNER_INIT_POLL_COUNTS) {
		//User_delay(Afatech_OMEGA_demodulator,OMEGA_TUNER_INIT_POLL_INTERVAL);
		User_delay(demodulator,OMEGA_TUNER_INIT_POLL_INTERVAL);
		error = omega_rmr(demodulator,0xED23, 2, &buf[2]);	/*r_reg_r_fbc_m_bdry_7_0*/
		if (error) goto exit;
		m_bdry = (buf[3]<<8) + buf[2];	/*m_bdry = m_bdry_15_8 << 8 + m_bdry_7_0*/
		cnt++;
    ...
    demod->gc_fN_min[7]= (unsigned int) (tmp_numer / tmp_denom);


	/*----- update c_fN_min[6] ~c_fN_min[1], by Hidy 11/17/11-----*/
    ...
    demod->gc_fN_min[6] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 6);
    demod->gc_fN_min[5] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 8);
    demod->gc_fN_min[4] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 12);
    demod->gc_fN_min[3] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 16);
    demod->gc_fN_min[2] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 24);
    demod->gc_fN_min[1] = (unsigned int) (tmp_numer / (unsigned long)g_fdiv/ 32);

	/*----- patch to wait for all cal done, borrow p_reg_p_tsm_init_mode -----*/
	cnt = 1;	   
	//error = Standard_readRegister(Afatech_OMEGA_demodulator, Processor_LINK, chip_version_7_0, &chip_version);
	error = Standard_readRegister(demodulator, Processor_LINK, chip_version_7_0, &chip_version);
	if (error) goto exit;
	//error = Standard_readRegisters(Afatech_OMEGA_demodulator, Processor_LINK, chip_version_7_0+1, 2, var);
	...
	if(chip_Type==0x9135 && chip_version == 2){			
		//User_delay(Afatech_OMEGA_demodulator,50);  //for omega2
		User_delay(demodulator,50);  //for omega2
	}else{
		error = omega_rmr(demodulator,0xEC82, 1, buf);	/*p_reg_p_tsm_init_mode*/
		if (error) goto exit;
	...
	/*----- write -----*/
	error = omega_wmr(demodulator,0xED81, 1, &val);		/*p_reg_p_iqik_m_cal*/
	if (error) goto exit;
	
	//error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh8_en, reg_top_gpioh8_en_pos, reg_top_gpioh8_en_len, 1);
	error = Standard_writeRegisterBits (demodulator, Processor_LINK, p_reg_top_gpioh8_en, reg_top_gpioh8_en_pos, reg_top_gpioh8_en_len, 1);
	if (error) goto exit;
	//error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh8_on, reg_top_gpioh8_on_pos, reg_top_gpioh8_on_len, 1);
	error = Standard_writeRegisterBits (demodulator, Processor_LINK, p_reg_top_gpioh8_on, reg_top_gpioh8_on_pos, reg_top_gpioh8_on_len, 1);
	if (error) goto exit;
...
unsigned long omega_setfreq(Demodulator*	demodulator,unsigned int bw_kHz, unsigned int rf_freq_kHz)
{
	...
	/*----- read -----*/
	error = omega_rmr(demodulator,0xEC4C, 1, buf);		/*p_reg_t_ctrl*/
	if (error) goto exit;
	error = omega_rmr(demodulator,0xED81, 1, &buf[1]);	/*p_reg_p_iqik_m_cal*/
	if (error) goto exit;
	...
	/*----- set_cal_freq -----*/
	tmp = omega_get_cal_freq_iqik(demodulator,rf_freq_kHz, buf[1]);
	val[3] = (unsigned char) (tmp & 0xFF);
	val[4] = (unsigned char) ((tmp >> 8) & 0xFF);

	/*----- set_lo_freq -----*/
	tmp = omega_get_lo_freq(demodulator,rf_freq_kHz);
	...
	error = Standard_writeRegisters (demodulator, Processor_OFDM, var_pre_lna_cap_sel, 1, val);
	if (error) goto exit;
	error = omega_wmr(demodulator,0xEC56, 1, &val[1]);	/*p_reg_t_lpf_bw*/
	if (error) goto exit;
	error = omega_wmr(demodulator,0xEC4C, 1, &val[2]);	/*p_reg_t_ctrl*/
	if (error) goto exit;
	/*----- the writing sequence is important for cal_freq and lo_freq, thus separate ----- */
	error = omega_wmr(demodulator,0xEC4D, 1, &val[3]);	/*p_reg_t_cal_freq_7_0*/
	if (error) goto exit;
	error = omega_wmr(demodulator,0xEC4E, 1, &val[4]);	/*p_reg_t_cal_freq_15_8*/
	if (error) goto exit;
	...
	error = Standard_writeRegisters (demodulator, Processor_OFDM, var_pre_lo_freq_7_0, 1, &val[5]);
	if (error) goto exit;
	...
	error = Standard_writeRegisters (demodulator, Processor_OFDM, var_pre_lo_freq_15_8, 1, &val[6]);
    if (error) goto exit;

    if(rf_freq_kHz<c_fUHF_min)
		//error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 0);
	    error = Standard_writeRegisterBits (demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 0);
	else
		//error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 1);
		error = Standard_writeRegisterBits (demodulator, Processor_LINK, p_reg_top_gpioh8_o, reg_top_gpioh8_o_pos, reg_top_gpioh8_o_len, 1);
	if (error) goto exit;
exit:
	return (error);
}
...
{	//demodulator
	return (Standard_readRegisters (
		demodulator, Processor_OFDM, mem_addr, len, buf));
}
...
unsigned long omega_wmr(Demodulator*	demodulator,unsigned long mem_addr, unsigned char len, unsigned char *val)
{
	return (Standard_writeRegisters (
		demodulator, Processor_OFDM, mem_addr, len, val));
}
...
unsigned int omega_get_cal_freq_iqik(Demodulator*	demodulator,unsigned int rf_freq_kHz, unsigned char iqik_m_cal)
{
	...
	cal_freq = omega_get_freq_code(demodulator,rf_freq_kHz, &nc, &nv, &mv);
...
unsigned int omega_get_freq_code(Demodulator*	demodulator,unsigned int rf_freq_kHz, unsigned int *nc, unsigned int *nv, unsigned int *mv)
{
	...
	*nc = omega_get_nc(demodulator,rf_freq_kHz);
	*nv = omega_get_nv(*nc);
...
unsigned int omega_get_nc(Demodulator*	demodulator,unsigned int rf_freq_kHz)
{
    ...
    if ((rf_freq_kHz <= demod->gc_fN_min[1]))										        {nc = 0;}	/*74*/
	else if ((rf_freq_kHz > demod->gc_fN_min[1]) && (rf_freq_kHz <= demod->gc_fN_min[2]))	{nc = 1;}	/*74 111*/	
	else if ((rf_freq_kHz > demod->gc_fN_min[2]) && (rf_freq_kHz <= demod->gc_fN_min[3]))	{nc = 2;}	/*111 148*/
	else if ((rf_freq_kHz > demod->gc_fN_min[3]) && (rf_freq_kHz <= demod->gc_fN_min[4]))	{nc = 3;}	/*148 222*/	
	else if ((rf_freq_kHz > demod->gc_fN_min[4]) && (rf_freq_kHz <= demod->gc_fN_min[5]))	{nc = 4;}	/*222 296*/	
	else if ((rf_freq_kHz > demod->gc_fN_min[5]) && (rf_freq_kHz <= demod->gc_fN_min[6]))	{nc = 5;}	/*296 445*/
	else if ((rf_freq_kHz > demod->gc_fN_min[6]) && (rf_freq_kHz <= demod->gc_fN_min[7]))	{nc = 6;}	/*445 573*/
	else if ((rf_freq_kHz > demod->gc_fN_min[7]) && (rf_freq_kHz <= demod->gc_fN_min[8]))	{nc = 7;}	/*573 890*/
	else 	        														                {nc = 8;}	/*L-band*/
	    return nc;
}
...
unsigned int omega_get_lo_freq(Demodulator*	demodulator,unsigned int rf_freq_kHz)
{
	...
	lo_freq = omega_get_freq_code(demodulator,rf_freq_kHz, &nc, &nv, &mv);
	return lo_freq;
}
