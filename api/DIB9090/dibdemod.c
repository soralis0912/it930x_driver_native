...
int demod_gpio_function(struct dibFrontend *fe, struct dibGPIOFunction *f)
{
	uint8_t i;
	if (f == NULL)
		return DIB_RETURN_ERROR;
	...
	for (i = 0; i < 32; i ++)
		if ((1 << i) & f->mask)
			if((demod_set_gpio(fe, i, (uint8_t)((f->direction >> i) & 0x1), (uint8_t)((f->value >> i) & 0x1)))!=DIB_RETURN_SUCCESS)
					return DIB_RETURN_ERROR;
