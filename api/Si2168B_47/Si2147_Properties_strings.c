...
void  Si2147_showProperty         (L1_Si2147_Context *api, int propIndex)
{
  ...
  for (fieldIndex=0; fieldIndex<Si2147_propertyDictionary[propIndex].nbFields; fieldIndex++)
  {
    printf("%s:%s:%s = ", Si2147_featureText[Si2147_propertyDictionary[propIndex].feature], Si2147_propertyDictionary[propIndex].name, Si2147_propertyDictionary[propIndex].field[fieldIndex].name);
...
int   Si2147_showProperties       (L1_Si2147_Context *api, Si2147_featureEnum feature)
{
  ...
  for (propertyIndex=0; propertyIndex<dictSize; propertyIndex++)
  {
    ...
    Si2147_showProperty(api, propertyIndex);
  }
  return 0;
}
...
#else  /* Si2147_COMMAND_LINE_APPLICATION */
  Si2147_setupCOMMONDefaults(api);
