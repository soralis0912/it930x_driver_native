...
#include "Si2147_firmware_3_0_build_x.h"       /* dummy firmware file for future patch replacement */
...
int Si2147_PowerUpWithPatch    (L1_Si2147_Context *api)
{
    ...
    if ((return_code = Si2147_pollForCTS  (api)) != NO_Si2147_ERROR) {
        SiTRACE ("Si2147_pollForCTS error 0x%02x\n", return_code);
        return return_code;
    }
    ...
    if ((return_code = Si2147_L1_PART_INFO(api)) != NO_Si2147_ERROR) {
        SiTRACE ("Si2147_L1_PART_INFO error 0x%02x: %s\n", return_code, Si2147_L1_API_ERROR_TEXT(return_code) );
        return return_code;
    }
    ...
    if (api->rsp->part_info.romid == 0x50)
    {
       ...
       if ((return_code = Si2147_LoadFirmware_16(api, Si2147_FW_3_0bx, FIRMWARE_LINES_3_0bx)) != NO_Si2147_ERROR) {
    ...
    if ((return_code = Si2147_StartFirmware(api)) != NO_Si2147_ERROR) { /* Start firmware */
        SiTRACE ("Si2147_StartFirmware error 0x%02x: %s\n", return_code, Si2147_L1_API_ERROR_TEXT(return_code) );
        return return_code;
    }
...
/* add check for user abort here */
  ...
  else
  {
    if (freq==previousFrequency)
      ...
      printk("Scanning Frequency %d, ",freq);
  }
