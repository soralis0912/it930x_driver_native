...
{
	return SONY_RESULT_ERROR_NOSUPPORT;
}
#endif
                                 ...
                                 sony_i2c_t * pDemodI2c)
{
    ...
    pDemod->serialTSClkFreqTerrCable = SONY_DEMOD_SERIAL_TS_TLV_CLK_MID_FULL;
#endif
#ifdef SONY_DEMOD_SUPPORT_SAT
    pDemod->serialTSClkFreqSat = SONY_DEMOD_SERIAL_TS_TLV_CLK_MID_FULL;
    ...
    pDemod->serialTLVClkFreqTerrCable = SONY_DEMOD_SERIAL_TS_TLV_CLK_MID_FULL;
    pDemod->serialTLVClkFreqSat = SONY_DEMOD_SERIAL_TS_TLV_CLK_MID_FULL;
    pDemod->twoBitParallelTLVClkFreqTerrCable = SONY_DEMOD_2BIT_PARALLEL_TLV_CLK_MID;
    pDemod->twoBitParallelTLVClkFreqSat = SONY_DEMOD_2BIT_PARALLEL_TLV_CLK_MID;
#endif
...
sony_result_t sony_demod_Initialize (sony_demod_t * pDemod)
{
    ...
    pDemod->scanMode = 0;
    pDemod->isdbtEwsState = SONY_DEMOD_ISDBT_STATE_NORMAL;
#endif
#ifdef SONY_DEMOD_SUPPORT_SAT
...
#ifdef SONY_DEMOD_SUPPORT_TLV
    if (((pDemod->system == SONY_DTV_SYSTEM_ISDBS3) && (pDemod->isdbs3TLVOutput))
        || ((pDemod->system == SONY_DTV_SYSTEM_DVBC2) && (pDemod->dvbc2TLVOutput))) {
        /* Set SLV-T Bank : 0x01 */
        ...
        /* Enable TLV output */
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xC0, 0x00) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    } else
#endif
                                    ...
                                    int32_t value)
{
    ...
    case SONY_DEMOD_CONFIG_TERR_CABLE_TS_SERIAL_CLK_FREQ:
        ...
        pDemod-> serialTSClkFreqTerrCable = (sony_demod_serial_ts_tlv_clk_t) value;
        break;
    ...
    case SONY_DEMOD_CONFIG_DVBTT2_AVESNR_PERIOD_TIME:
        /* This flag can change only in Sleep state */
        if (pDemod->state != SONY_DEMOD_STATE_SLEEP) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
        }
    ...
    case SONY_DEMOD_CONFIG_SAT_TS_SERIAL_CLK_FREQ:
        ...
        pDemod->serialTSClkFreqSat = (sony_demod_serial_ts_tlv_clk_t) value;
        break;
    ...
    case SONY_DEMOD_CONFIG_TLV_OUTPUT_ISDBS3:
        /* This register can change only in Sleep state */
        if (pDemod->state != SONY_DEMOD_STATE_SLEEP) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
        }
    ...
    case SONY_DEMOD_CONFIG_TLV_OUTPUT_DVBC2:
        /* This flag can change only in Sleep state */
        if (pDemod->state != SONY_DEMOD_STATE_SLEEP) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
        }
    ...
    case SONY_DEMOD_CONFIG_TERR_CABLE_TLV_SERIAL_CLK_FREQ:
        ...
        /* Store the clock frequency mode for terrestrial and cable systems */
        pDemod-> serialTLVClkFreqTerrCable = (sony_demod_serial_ts_tlv_clk_t) value;
        break;
    ...
    case SONY_DEMOD_CONFIG_SAT_TLV_SERIAL_CLK_FREQ:
        ...
        /* Store the clock frequency mode for satellite systems */
        pDemod->serialTLVClkFreqSat = (sony_demod_serial_ts_tlv_clk_t) value;
        break;
    ...
    case SONY_DEMOD_CONFIG_TERR_CABLE_TLV_2BIT_PARALLEL_CLK_FREQ:
        ...
        /* Store the clock frequency mode for terrestrial and cable systems */
        pDemod->twoBitParallelTLVClkFreqTerrCable = (sony_demod_2bit_parallel_tlv_clk_t) value;
        break;
    ...
    case SONY_DEMOD_CONFIG_SAT_TLV_2BIT_PARALLEL_CLK_FREQ:
        ...
        /* Store the clock frequency mode for satellite systems */
        pDemod->twoBitParallelTLVClkFreqSat = (sony_demod_2bit_parallel_tlv_clk_t) value;
        break;
...
sony_result_t sony_demod_SetTLVClockModeAndFreq (sony_demod_t * pDemod, sony_dtv_system_t system)
{
     ...
     */
    if ((system == SONY_DTV_SYSTEM_ISDBS3) && (serialTLV & 0x80)){
        result = sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE7, 0, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    } else {
        result = sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE7, 1, 0x01);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
    }
    ...
    if (serialTLV & 0x80) {
        /* Serial TLV */
        if (system == SONY_DTV_SYSTEM_DVBC2) {
            /* Terrestrial or Cable */
     ...
     * <SLV-T>  00h     33h     [1:0]  2'b01      OREG_CKSEL_TSTLVIF
     */
    result = sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x33, tlvClkConfiguration.clkSelTLVIf, 0x03);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
sony_result_t sony_demod_SetTSDataPinHiZ (sony_demod_t * pDemod, uint8_t enable)
{
    ...
    if (sony_i2c_SetRegisterBits (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x81, (uint8_t) (enable ? 0xFF : 0x00), tsDataMask) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
